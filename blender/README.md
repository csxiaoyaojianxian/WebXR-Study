...
`G` + `X/Y/Z`: 移动，按两下坐标切换局部坐标系！按下 `ctrl` 吸附网格移动
...
`F`：填充封闭，根据顶点补充面，选中线后按F生成面，也可以选择线后，菜单面=>栅格填充，或菜单顶点=>从顶点创建边/面
...
`L`: 选中相连元素，即选中点线面时，选择与当前选中目标相连的物体
...
**编辑模式**，编辑骨架，可以细分骨骼、从骨架E挤出

**姿态模式**，对骨骼摆姿势，用于做动画切换姿态，旋转移动骨架，查看骨架父子关系是否正确，制作关键帧动画

姿态模式下清空变换：`A` 全选，右键清空姿态变换

注意：物体模式只能给骨架打关键帧，姿态模式才能给骨骼打关键帧

**权重模式**，选中骨骼下绑定过骨骼的模型，切换模式至权重绘制，右侧"数据"->"顶点组"检查每根骨骼的权重，即蓝色到红色的渐变
...
挤出骨骼： `E` 挤出创建骨骼，注意：小头挤出创建关联骨骼(子集)，大头挤出创建**没有关联的骨骼**(平级)，父子级关系会影响姿态设置
...
骨骼绑定，必须在**物体模式**下进行，模型需要作为骨架的子集，先选中物体(如环切后的圆柱体)再选中骨架，`cmd` + `P` =>骨架形变(附带自动权重) 
